...
<div>
	...
	{:else}
		<!-- Timeline mode with sub-agents: indent-based -->
		<div>
			{#each displayLaneEvents as item, idx}
				{#if 'kind' in item && item.kind === 'collapsed'}
					...
					<div style="padding-left: {depth * LANE_WIDTH_PX}px">
						<button
						...
						>
							...
							<span class="shrink-0 font-mono text-[10px] text-text-muted">
								{info.eventCount} events &middot; {formatMs(info.durationMs)}
							</span>
						</button>
					</div>
				{:else if 'kind' in item && item.kind === 'paired'}
					...
					<div style="padding-left: {depth * LANE_WIDTH_PX}px" class:opacity-60={isLowPriority}>
						{#if depth > 0}
						...
						{#if depth > 0}
							<div style="border-left: 4px solid {getLaneColor(depth)}; padding-left: 0px">
								<div class="flex items-center gap-2 border border-border/30 bg-bg-secondary/50 px-3 py-1.5 text-xs">
									<span class="shrink-0 rounded bg-text-muted/10 px-1.5 py-0.5 font-mono text-[10px] font-semibold text-text-muted">
										{group.count}&times;
									</span>
									<span class="shrink-0 font-medium text-text-secondary">
										{consecutiveGroupDisplayName(group.groupKey)}
									</span>
									<span class="min-w-0 flex-1 truncate font-mono text-[10px] text-text-muted">
										{consecutiveGroupSummary(group.events)}
									</span>
								</div>
							</div>
						{:else}
				...
				{:else}
					...
					{#if depth === 0 && previousRole && previousRole !== currentRole}
						{@render roleSeparator(currentRole)}
					{/if}
					<div style="padding-left: {depth * LANE_WIDTH_PX}px" class:opacity-60={isLowPriorityInLane} data-timeline-idx={idx}>
						{#if laneEvent.event.event_type.type === 'TaskStart' && laneEvent.event.task_id}
							...
							>
								...
								<span class="flex-1 font-semibold text-text-primary truncate">
									{'data' in laneEvent.event.event_type ? (laneEvent.event.event_type.data as { title?: string }).title || 'Sub-agent' : 'Sub-agent'}
								</span>
								{#if taskInfo}
									<span class="shrink-0 font-mono text-[10px] text-text-muted">{taskInfo.eventCount} events &middot; {formatMs(taskInfo.durationMs)}</span>
