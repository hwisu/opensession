...
impl LocalDb {
    ...
    pub fn get_sessions_by_tool_latest(
        ...
        let conn = self.conn();
        let mut stmt = conn.prepare(&sql)?;
        let rows = stmt.query_map(params![tool, count], row_to_local_session)?;
        let mut result = Vec::new();
        for row in rows {
            result.push(row?);
        }
    ...
    pub fn get_sessions_latest(&self, count: u32) -> Result<Vec<LocalSessionRow>> {
        ...
        let conn = self.conn();
        let mut stmt = conn.prepare(&sql)?;
        let rows = stmt.query_map(params![count], row_to_local_session)?;
        let mut result = Vec::new();
        for row in rows {
            result.push(row?);
        }
    ...
    pub fn get_session_by_tool_offset(
        ...
        let mut stmt = conn.prepare(&sql)?;
        let row = stmt
            .query_map(params![tool, offset], row_to_local_session)?
            .next()
            .transpose()?;
        Ok(row)
    }
    ...
    pub fn get_session_by_offset(&self, offset: u32) -> Result<Option<LocalSessionRow>> {
        let sql = format!(
            "SELECT {LOCAL_SESSION_COLUMNS} \
             {FROM_CLAUSE} \
             ORDER BY s.created_at DESC LIMIT 1 OFFSET ?1"
        );
        let conn = self.conn();
        let mut stmt = conn.prepare(&sql)?;
        let row = stmt
            .query_map(params![offset], row_to_local_session)?
            .next()
            .transpose()?;
        Ok(row)
    }
