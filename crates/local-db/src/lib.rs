...
/// Return true for likely synthetic Codex rows generated by internal orchestration.
fn is_codex_internal_session(row: &LocalSessionRow) -> bool {
    if row.tool != "codex" {
        return false;
    }
...
pub fn hide_opencode_child_sessions(mut rows: Vec<LocalSessionRow>) -> Vec<LocalSessionRow> {
    rows.retain(|row| {
        !is_opencode_child_session(row)
            && !is_codex_internal_session(row)
            && !is_claude_subagent_session(row)
    });
    rows
}
...
impl LocalDb {
    ...
    fn build_local_session_where_clause(
        ...
        let mut where_clauses = vec![
            "1=1".to_string(),
            "NOT (s.tool = 'codex' AND COALESCE(s.user_message_count, 0) = 0)".to_string(),
...
mod tests {
    ...
    #[test]
    fn test_is_codex_internal_session() {
        let hidden = make_row("codex_internal", "codex", None);
        assert!(is_codex_internal_session(&hidden));

        let mut visible = make_row("codex_visible", "codex", None);
        visible.user_message_count = 1;
        assert!(!is_codex_internal_session(&visible));
    }
