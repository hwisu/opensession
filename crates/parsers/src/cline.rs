use crate::common::{
    build_tool_result_content, extract_tag_content, strip_system_reminders, ToolUseInfo,
};
...
static TOOL_RESULT_PREFIX_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"^\[(\w+)(?:\s+for\s+'([^']*)')?\]\s+Result:\n?").unwrap()
});
...
fn find_task_title(task_dir: &Path, task_id: &str) -> Option<String> {
    let state_dir = task_dir
        .parent()? // tasks/
        .parent()? // data/
        .join("state")
        .join("taskHistory.json");

    if !state_dir.exists() {
        return None;
    }

    let text = std::fs::read_to_string(&state_dir).ok()?;
